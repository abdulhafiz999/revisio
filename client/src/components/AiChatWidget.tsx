...
import { Sparkles, X, Send, Loader2, ChevronDown, Bot, Maximize2, Minimize2, AlertTriangle, RefreshCw } from 'lucide-react';
import { cn } from '@/lib/utils';
import { apiClient, ChatMessage } from '@/services/api.client';
import { useToast } from '@/hooks/use-toast';
import { getFriendlyErrorMessage } from './AIErrorDialog';
...
export function AiChatWidget() {
  ...
  const handleRetry = async () => {
    ...
    const userMessages = messages.filter((m: any) => !m.isError);
    if (userMessages.length <= 1) return; // Only welcome message left
    ...
    try {
      ...
      const conversationToSend = userMessages.slice(1);
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: errMsg, isError: true } as any
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSend = async () => {
    ...
    // Clear any previous error messages when sending a new one
    const updatedMessages = [...messages.filter((m: any) => !m.isError), userMessage];
            ...
            <>
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 scroll-smooth">
                {messages.map((msg, idx) => {
                  const isMsgError = (msg as any).isError;
                  return (
                    <div
                      key={idx}
                      className={cn(
                    ...
                    >
                      ...
                      {isMsgError ? (
                        <div
                          className={cn(
                            'max-w-[85%] rounded-2xl p-4 border text-left',
                            'bg-orange-500/10 border-orange-500/20 text-foreground rounded-bl-sm'
                          )}
                        >
                          <div className="flex gap-2.5">
                            ...
                            <div className="space-y-2">
                              <p className="text-sm font-semibold text-orange-500 leading-tight">
                                {msg.content.toLowerCase().includes('rate limit') || msg.content.toLowerCase().includes('429') || msg.content.toLowerCase().includes('quota')
                                  ? 'Revi is taking a quick breath ☕'
                                  : "Revi's brain is a bit crowded 🧠"}
                              </p>
                              <p className="text-xs text-muted-foreground leading-relaxed">
                                {getFriendlyErrorMessage(msg.content)}
                              </p>
                              <div className="flex gap-3 pt-1">
                                ...
                                <button
                                  onClick={() => {
                                    // Retrieve the text of the user's message
                                    ...
                                    // Remove the user message and error message from history
                                    setMessages((prev) =>
                                      prev.filter((_, i) => i !== idx && i !== idx - 1)
                                    );
                                  }}
